    ...
    <DashboardFilterProvider label={timeRangeLabel}>
      <div className="content">
        <section className="page active" id="page-dashboard">
          ...
          <div className="bento-row">
            <div className="surface hero-overview">
              ...
              <div className="seg-bar">
                <span
                  style={{
                    width: `${(assignedAssets / Math.max(1, totalAssets)) * 100}%`,
                    background: "#01DB5E",
                  }}
                ></span>
                <span
                  style={{
                    width: `${(unassignedAssets / Math.max(1, totalAssets)) * 100}%`,
                    background: "#05E0E5",
                  }}
                ></span>
                <span
                  style={{
                    width: `${(damagedAssets / Math.max(1, totalAssets)) * 100}%`,
                    background: "#FF6857",
                  }}
                ></span>
              </div>
              <div className="seg-legend">
                <div className="seg-legend-item">
                  <div className="sw" style={{ background: "#01DB5E" }}></div> Assigned{" "}
                  <b>{assignedAssets}</b>
                </div>
                <div className="seg-legend-item">
                  <div className="sw" style={{ background: "#05E0E5" }}></div> Unassigned{" "}
                  <b>{unassignedAssets}</b>
                </div>
                <div className="seg-legend-item">
                  <div className="sw" style={{ background: "#FF6857" }}></div> Damaged{" "}
                  <b>{damagedAssets}</b>
                </div>
              </div>
            </div>
          ...
          <div className="trio-row">
            <div className="surface panel">
              ...
              {isLoadingAssets ? (
                ...
                <div className="donut-wrap">
                  <PieChart width={130} height={130}>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {devicesAvailabilityData.map((entry, i) => (
                  ...
                  <div className="legend">
                    {devicesAvailabilityData.map((d, i) => (
                      <div key={i} className="legend-item">
                        <span className="sw" style={{ background: d.color }}></span> {d.name}{" "}
                        <b>{d.value}%</b>
                      </div>
                    ))}
                  </div>
                ...
                <div className="px-4 pb-4">
                  <DashboardEmptyState message="No data" height={130} />
                </div>
              )}
            </div>
              ...
              {isLoadingSecurity ? (
                ...
                <div className="donut-wrap">
                  <PieChart width={130} height={130}>
                    <Pie
                      data={
                        securityPatchData.every((d) => d.value === 0)
                          ? [{ name: "Unknown", value: 1, color: "var(--card-line)" }]
                          : securityPatchData
                      }
                      cx="50%"
                      cy="50%"
              ...
              <div className="quick-actions">
                <button className="quick-action-btn cursor-pointer">
                  ...
                  >
                    ...
                    <path d="M9 7h6M12 16l0-6M9 13h6" />
                  </svg>
                  Enrol a device
                </button>
                <button className="quick-action-btn cursor-pointer">
          ...
          <div className="surface map-banner relative overflow-hidden p-0">
            <div className="panel-head absolute top-0 z-10 w-full border-none bg-transparent px-4 pt-4">
              <h3>Device locations</h3>
              <div className="icon-btn cursor-pointer border border-[var(--card-line)] bg-[var(--surface)]/80 text-[var(--text-1)] backdrop-blur-md">
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
              ...
              {isLoadingWebsites ? (
                ...
                <table>
                  ...
                  <tbody>
                    {websites.map((v) => (
                      <tr key={v.domain}>
                        <td className="pl-4 font-medium">{v.domain}</td>
              ...
              {isLoadingAssets ? (
                ...
                <table>
                  ...
                  <tbody>
                    {lostReports.map((v, i) => (
                      <tr key={i}>
                        <td className="pl-4 font-medium">{v.device}</td>
                        <td>
                    ...
                    <tr key={device.id}>
                      ...
                      <td>
                        <span
                          className={cn(
                            "status-pill",
                            device.deviceStatus === "ACTIVE" ? "online" : "locked"
                          )}
                        >
                          <span className="dot"></span>
                      ...
                      <td className="mono">
                        {device.mdmLastSyncAt
                          ? format(new Date(device.mdmLastSyncAt), "dd MMM, HH:mm")
                          : "N/A"}
                      </td>
                    </tr>
                  ))
