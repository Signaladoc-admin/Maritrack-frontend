    ...
    <div className={cn("flex gap-2", className)}>
      {Array.from({ length }).map((_, i) => (
        <input
          ...
          onPaste={handlePaste}
          className={cn(
            "border-input bg-muted/20 aspect-square h-auto w-full rounded-md border-[1.5px] text-center text-xl sm:text-2xl font-bold transition-all outline-none focus:border-primary focus:ring-1 focus:ring-primary/40 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 max-h-24",
            value[i] ? "bg-background border-primary/65" : "border-input border"
          )}
        />
      ))}
    </div>
